"""Unit tests for the semantic search tool wrapper."""
...
from src.tools.search_tools import run_semantic_search_tool
...
class FakeSemanticModel:
    """Deterministic embedding stub for semantic tool tests."""
...
class SemanticSearchToolTests(unittest.TestCase):
    """Validate the standardized semantic tool contract."""
    ...
    def test_valid_semantic_query_returns_success_structure(self) -> None:
        """Return standardized success output with preserved semantic results."""
        with patch("src.retrieval.semantic._get_numpy_module", return_value=self.fake_numpy):
            response = run_semantic_search_tool("faiss index", self.model, self.faiss_index, self.metadata, top_k=2)

        self.assertEqual(response["tool_name"], "semantic_search")
        ...
        self.assertIn("FAISS index stores vectors.", response["results"][0]["matched_sentences"])
        self.assertEqual(response["results"][0]["snippet"], "FAISS index stores vectors.")

    def test_empty_query_returns_success_with_no_results(self) -> None:
        """Treat an empty query as a safe no-result success response."""
        with patch("src.retrieval.semantic._get_numpy_module", return_value=self.fake_numpy):
            response = run_semantic_search_tool("", self.model, self.faiss_index, self.metadata, top_k=5)
    ...
    def test_no_results_returns_success_with_ranked_zero_scores(self) -> None:
        """Preserve semantic-search output even when similarity is effectively zero."""
        zero_index = FakeFaissIndex(vectors=[[0.0, 0.0], [0.0, 0.0]])
        zero_metadata = [self.metadata[0], self.metadata[2]]

        with patch("src.retrieval.semantic._get_numpy_module", return_value=self.fake_numpy):
            response = run_semantic_search_tool("unknown topic", self.model, zero_index, zero_metadata, top_k=2)
    ...
    def test_invalid_faiss_input_returns_error_structure(self) -> None:
        ...
        with patch("src.retrieval.semantic._get_numpy_module", return_value=self.fake_numpy):
            response = run_semantic_search_tool("faiss index", self.model, BrokenFaissIndex(), self.metadata, top_k=2)
    ...
    def test_missing_metadata_returns_success_with_no_results(self) -> None:
        """Treat missing metadata as a safe no-result semantic response."""
        with patch("src.retrieval.semantic._get_numpy_module", return_value=self.fake_numpy):
            response = run_semantic_search_tool("faiss index", self.model, self.faiss_index, [], top_k=2)
